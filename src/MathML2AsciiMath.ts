...
class MathML2AsciiMath {
  convert(html: string): string {
    ...
    return this._parse(dom.window.document.documentElement)
      .replace(/\s+/g, " ")
      .trim();
  }
  ...
  _parse(node: ChildNode) {
    ...
    switch (node.nodeName) {
      ...
      case "mrow":
        ...
        if (
          ["mfrac", "msub", "munder", "munderover"].includes(
      ...
      case "mfrac":
        return `(${this._joinParsedChildren(
          node.childNodes[0],
          " "
        )})/(${this._joinParsedChildren(node.childNodes[1])})`;
      case "msup":
  ...
  _joinParsedChildren(
    children: NodeListOf<ChildNode> | ChildNode,
    delimiter = " "
  ) {
